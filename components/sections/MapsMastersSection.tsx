...
function getCellColor(value: number | null, allValues: (number | null)[]): string {
  ...
  if (defined.length <= 1) return 'text-gray-200';
...
export function MapsMastersSection({ stats, maps, teamLogos = {}, teamRegions = {}, mapImages = {} }: Props) {
  ...
  // Por defecto todos los mapas visibles
              ...
              <tr key={team} className="hover:bg-[#252a33] transition-colors border-b border-gray-800">
                ...
                {visibleMaps.map((m, mi) => {
                  ...
                  return (
                    <td
                      key={m}
                      className={`py-3 px-3 text-center ${isActive ? 'bg-[#1e2430]' : 'bg-[#1a1d23]'}`}
                      style={{ minWidth: 64 }}
                    >
                      {val !== null && wl ? (
                        <>
                          <div className={`text-sm ${color}`}>
                            {val}%
                            {showDetail && <span className="text-gray-600 font-normal whitespace-nowrap"> {wl.played} Ply - {wl.bans} Ban</span>}
                          </div>
                          {showDetail && (
                            <div className="text-[13px] text-gray-600 whitespace-nowrap">
                              {wl.wins}W-{wl.played - wl.wins}L
                            </div>
                          )}
                        </>
                      ) : showDetail && wl && wl.bans > 0 ? (
                        <div className="text-[11px] text-gray-600 font-normal whitespace-nowrap">
                          {wl.bans} Ban
                        </div>
                      ) : (
